...
static class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        foreach (var item in SumOfDaylyAmount)
            Console.WriteLine($"Date: {item.Date.ToShortDateString}, Total Amount: {item.SumAmount}");
        ...
        var label = KMeans.Cluster(AccNumbers, data, 9);
        ...
        string cardNumber = "898548464659599550001";
        if (LuhnAlgo.CheckByLuhn(cardNumber))
        ...
        foreach (var items in output_Balance)
        {
        ...
        Console.Write("Enter source currency: ");
        string fromCurrency = Console.ReadLine().ToUpper();

        Console.Write("Enter target currency: ");
        string toCurrency = Console.ReadLine().ToUpper();

        decimal convertedAmount = await ExchangeRates.ConvertCurrencyAsync(fromCurrency, toCurrency, amount);
